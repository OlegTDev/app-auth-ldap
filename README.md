...
2. Микросервис выполняет аутентификацию средствами веб-сервера, извлекает учетную запись пользователя из серверных параметров (`REMOTE_USER`).
3. В случае запроса ldap выполняет поиск расширенных данных о пользователе на сервере LDAP (ФИО, почта, отдел, группы).
3. Микросервис формирует защищенный `JWT-токен` с логином/данными пользователя и выполняет обратную переадресацию на указанный `return_url`, передавая токен в GET-параметре `token`.

### API

- `/auth/basic?return_url=http://example.com` - для аутентификации без поиска пользователя на LDAP сервере.
- `/auth/ldap?return_url=http://example.com` - для аутентификации с поиском пользователя на LDAP сервере.
